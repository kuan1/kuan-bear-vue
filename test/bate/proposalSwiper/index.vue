<template>
  <swiper v-if="pages.length" class="batch-swiper" :options="swiperOption" ref="mySwiper">
    <swiper-slide v-for="(item, key) in pages" :key="item.id">
      <div class="full-content" :style="item.background ? {backgroundImage: 'url(' + item.background + ')'} : {}">
        <div class="full-wrapper" :style="wrapperStyle">
          <component :isShow="pageIndex === key" v-for="(it, i) in item.layers" :is="it.is || 'material'" :key="i" :item="it" :class="['layer', it.class]" :data-test="key" :style="{zIndex: i + 100}"></component>
        </div>
      </div>
    </swiper-slide>
  </swiper>
